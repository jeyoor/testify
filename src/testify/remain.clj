...
(defmacro do-redis [& stmts]
    `(let [cred# (redis-creds)]
        (redis/with-server
            {:host (get cred# "hostname") :port (get cred# "port") :password (get cred# "password") :db 0}
             (let
                 [final#
                    (do
                    ~@stmts
...
(defn sstore [sname val]
    (do-redis
        (redis/sadd sname val)
    )
)

(defn sdelete [sname val]
    (do-redis
        (redis/srem sname val)
    )
)

(defn hstore [hname & valpairs]
    (do-redis
        (apply redis/hmset hname valpairs)
    )
)
