...
(defn content-transform
    ...
    [nodes [id content]]
    (html/transform nodes [(keyword (str "#" id))] (maybe-content content))
)
...
(defn subsw [nodes subs]
    (let [      ;replace : with ""
         keyz (map #(util/strip-colon (str %))  (keys subs))
         valz (vals subs)
...
;TESTING
(def node3 (html/html-snippet "<p id=name class=text>Namehere </p> <img src=cool /> <p> other tag </p> <img id=face src=blag.jpg class=image /> <video id=smile class=video ") )
;TODO
(defn testy []
