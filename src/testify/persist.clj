...
;TODO smarter create (replace this)
(defn hashify-thing [cat name field & hashpairs]
    (do-redis
        (apply redis/hmset (str cat ":" name field) hashpairs)
    )
)
(defn grab-hashy [cat name field]
    (do-redis
        (redis/hgetall (str cat ":" name field))
    )
)
...
;start page-specific
(defn page-fields []
        ;BEWARE HIDDEN FIELD SUBS
        ;TODO smarter create
        ;TODO add to restore,delete,get
        [":template",":token"]
)

(defn page-fieldmap [fieldvals]
    ;DEBUG
    ;(println (count fieldvals) " " (count (page-fields)))
...
(defn create-page [name submap & fieldvals]


    (apply create-thing "page" name (page-fieldmap fieldvals))
    ;TODO smarter create
    ;the interleave is flattening the hash to a list
    (apply hashify-thing "page" name ":subs"
        (interleave (keys submap) (vals submap))
    )
)

(defn update-page [name submap & fieldvals]
    (apply update-thing "page" name (page-fieldmap fieldvals))
    ;TODO smarter create-thing
    ;the interleave is flattening the hash to a list
    (apply hashify-thing "page" name ":subs"
        (interleave (keys submap) (vals submap))
    )
...
(defn template-fieldmap [& fieldvals]
...
(defn create-template [name & fieldvals]
    (apply create-thing "template" name (template-fieldmap fieldvals))
)

(defn update-template [name & fieldvals]
    (apply update-thing "template" name (template-fieldmap fieldvals))
)

(defn get-template [name]
    (apply get-thing "template" name (template-fields))
)

(defn delete-template [name]
    (apply delete-thing "template" name (template-fields))
)
